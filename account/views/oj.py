...
from ..serializers import (TwoFactorAuthCodeSerializer, UserProfileSerializer, StudentCourseRequestSerialzier,
                           EditUserProfileSerializer, ImageUploadForm)
from ..tasks import send_email_async
from course.serializers import CourseSerializer, CourseToAddSerializer
from course.models import Course, JoinCourseRequest
...
class UsernameOrEmailOrSnoCheck(APIView):
    @validate_serializer(UsernameOrEmailCheckSerializer)
    def post(self, request):
        """
        ...
        if data.get("sno"):
            result["sno"] = User.objects.filter(sno=data["sno"]).exists()
...
class UserRegisterAPI(APIView):
    ...
    def post(self, request):
        ...
        user = User.objects.create(username=data["username"], email=data["email"], sno=data["sno"])
...
class UserCourseAPI(APIView):
    ...
    def get(self, request):
        user = request.user
        is_selected = request.GET.get("is_selected") == "1"
        try:
            stu = User.objects.get(id=user.id)
        except User.DoesNotExist:
            return self.error("User does not exist")
        if is_selected:
            courses = Course.objects.filter(students=stu).order_by("-id")
            return self.success(self.paginate_data(request, courses, CourseSerializer))
        else:
            courses = Course.objects.exclude(students=stu).order_by("-id")
            for course in courses:
                if course.join_course_requests.filter(user=stu).exists():
                    try:
                        join_request = course.join_course_requests.get(user=stu)
                    except JoinCourseRequest.DoesNotExist:
                        return self.error("JoinCourseRequest error")
                    course.is_exist = True
                    course.status = join_request.status
                else:
                    course.is_exist = False
                    course.status = False
            return self.success(self.paginate_data(request, courses, CourseToAddSerializer))

    @validate_serializer(StudentCourseRequestSerialzier)
    @login_required
    def post(self, request):
        data = request.data
        user = request.user
        ...
        if is_selected == "1":
            ...
            join_requests = JoinCourseRequest.objects.filter(user=stu, course=course)
            for jrs in join_requests:
                jrs.delete()
        elif is_selected == "0":
            try:
                JoinCourseRequest.objects.create(user=stu, course=course)
            except IntegrityError:
                self.error("You had requested for this course")
        else:
            self.error("is_selected wrong")
        return self.success()