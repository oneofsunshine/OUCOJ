...
class ContestListAPI(APIView):
    ...
    def get(self, request):
        ...
        contests = Contest.objects.filter(id=-1)
        for course in courses:
            contest = course.contests.select_related("created_by").filter(visible=True)
            contests = contests | contest
        contests = contests.distinct()
    ...
    def get_rank(self):
        if self.contest.rule_type == ContestRuleType.ACM:
            return ACMContestRank.objects.filter(contest=self.contest,
                ...
                select_related("user").order_by("-accepted_number", "submission_number")
        else:
